function [Wl,gamma] =  cv_solver_L21ridge(y,Xl,Xlr,lambda1,lambda2,G_init,maxiter,toler)
...
% nested function
    ...
    function [fval] = primal_fval(y,Xl,Xlrcsi,Wl,gamma,lambda1,lambda2)
        ...
        fval = fval + rmse + lambda1 * sum(sqrt(sum(Wl.^2, 2))) + lambda2 * norm(gamma)^2;
    ...
    function gamma = solve_G(y,Xl,Xlrcsi,Wl_old,lambda2)
        ...
        y_all = cat(1,y_all{:});
        X_all = cat(1,Xlrcsi{:});
        gamma = ridge(y_all,X_all,lambda2);
