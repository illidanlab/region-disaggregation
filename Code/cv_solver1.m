function [b,Wl,gamma] =  cv_solver1(y,Xl,Xlr,lambda1,lambda2,G_init,maxiter,toler)
...
%     [~, sampleN, ~, ~] = diagonalize(Xl, y);
    ...
    fval(1) = primal_fval(y,Xltmp,Xlrcsi,Wlinit,gamma_init,randn(r,1),lambda1,lambda2);
    %BCD starts here
    gamma_old = gamma_init;
    for i = 2:maxiter
        %fix Xlr, G, solving Wl,Wr;
        [b,Wl] = solve_WlWr(y,Xltmp,Xlrcsi,gamma_old,lambda1);
        ...
        gamma = solve_G(ytmpymp,Xltmp,Xlrcsi,Wl_old,lambda2);
        gamma_old = gamma;
        fval(i,1) = primal_fval(y,Xltmp,Xlrcsi,Wl,gamma,b,lambda1,lambda2);
...
% nested function
    %% 
    function [fval] = primal_fval(y,Xl,Xlrcsi,Wl,gamma,b,lambda1,lambda2)
        fval = 0;
        
        for region = 1:r
            rmse = norm(y{region}-Xl{region}*Wl(:,region) - Xlrcsi{region}*gamma - b(region));
            
        end
        fval = fval + rmse + lambda1 * l1_mat(Wl) + lambda2 * l1_mat(gamma);
      
    end

    function [b,W] = solve_WlWr(y,Xl,Xlr,G_old,lambda1)
        ...
        for region = 1:r
            ...
            Xl{region} = horzcat(ones(size(Xl{region},1),1),Xl{region});
        end
